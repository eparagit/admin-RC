...
class BookingController extends Controller
{
    ...
    public function RejectBooking(Request $request){
      ...
      $subscribers=DB::select("select r.Cantidad_Personas, v.ID_Viaje, v.Cupo from reservacion r, viaje v where ID_Reservacion='".$id_book."' and r.viaje_ID=v.ID_Viaje");
      $array_sub= json_decode(json_encode($subscribers), True);

      $cupo=$array_sub[0]['Cantidad_Personas']+$array_sub[0]['Cupo'];

      $result_update_tour=DB::update("update viaje set Cupo='".$cupo."' where ID_Viaje='".$array_sub[0]['ID_Viaje']."'");
    ...
    public function NotifyBookingRejected(Request $request){
        ...
        foreach($array as $book){
          ...
          $name="Ruta a la Cima";
          $email='rutaalacima@gmail.com';
          $messageEmail = "Estimado ".$uname." ".$justification.".Del viaje: ".$booking;
